...
Hablando en términos más puntuales tendríamos para soportar nuestra aplicación mediante Sockets Stream (TCP) o Datagram Sockets (UDP). Ahora si partimos de que cada tipo respeta las funcionalidades mencionadas anteriormente de acuerdo a cada protocolo, decidimos que el uso de Stream Socket(TCP) es más apropiado:
